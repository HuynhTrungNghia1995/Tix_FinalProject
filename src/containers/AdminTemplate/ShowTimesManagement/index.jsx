...
export default function ShowTimesManagement() {
    ...
    const renderData = () => {
        if (showTimesFilm) {
                    ...
                    return cumRap.lichChieuPhim.map((lichChieu, index) => (
                        <StyledTableRow key={index}>
                            <StyledTableCell align="center">
                                <img style={{ height: 30, width: "auto" }} src={item.logo} />
                            </StyledTableCell>
                            <StyledTableCell align="center">{item.maHeThongRap}</StyledTableCell>
                            <StyledTableCell align="center">{cumRap.tenCumRap}</StyledTableCell>
                            <StyledTableCell align="center">{lichChieu.tenRap}</StyledTableCell>
                            <StyledTableCell align="center">{lichChieu.ngayChieuGioChieu}</StyledTableCell>
                            <StyledTableCell align="center">{lichChieu.giaVe}</StyledTableCell>
                        </StyledTableRow>
                    ))
                }
                )
            ...
            return <StyledTableRow><StyledTableCell colSpan={12} align="center"><h5 className="text-center">Chọn phim để coi thông tin rạp</h5></StyledTableCell></StyledTableRow>
        }
    ...
    const renderCinemaRoom = () => {
        if (cinemaList) {
            return cinemaList.map((item) => {
                if (item.maCumRap === cinemaItem) {
                    return item.danhSachRap.map((room) => {
                        return (
                            <option key={room.maRap} value={`${room.maRap}`}> {room.tenRap}</option>
                        )
                    })
                }
            })
    ...
    return (
        <div>
            ...
            <Grid container>
                <Grid item xs={12}>
                    <FormControl variant="outlined" style={{ width: 500 }} className={classes.formControl}>
                ...
                <Grid item xs={12} className="mt-1">
                    <FormControl variant="outlined" style={{ width: 200 }} className={classes.formControl}>
                ...
                <Grid item xs={12} className="mt-1">
                    <FormControl variant="outlined" style={{ width: 360 }} className={classes.formControl}>
                ...
                <Grid item xs={6} className="mt-1">
                    ...
                    <FormControl variant="outlined" className={classes.formControl} style={{ width: 180 }}>
                        <InputLabel htmlFor="outlined-age-native-simple">Giá Tiền</InputLabel>
                        <Select
                            native
                            label="Giá Tiền"
                ...
                <Grid item xs={12}>
                    <Button className={classes.formControl} onClick={handleAddShowTime} disabled={disableSubmit} variant="contained">Tạo Lịch Chiếu</Button>
                </Grid>
